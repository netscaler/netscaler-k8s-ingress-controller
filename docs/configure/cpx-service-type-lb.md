...
7. Create a YAML file with the required configuration for service of type LoadBalancer.
    ...
    **Note:** For detailed information, see [service configuration](#Service-Configuration). The service configuration section   explains different ways to get an external IP address for the service and also how to use the service annotation provided by Citrix to configure different Netscaler functionalities.
...
You can use Helm charts to install the Netscaler CPX as BGP router. For more information, see the [Citrix Helm chart documentation](https://github.com/citrix/citrix-helm-charts).
