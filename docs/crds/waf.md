...
The WAF CRD is available in the Citrix ingress controller GitHub repository at [waf-crd.yaml](../../crd/waf/waf-crd.yaml). The WAF CRD provides attributes for the various options that are required to define the web application firewall policies on Citrix ADC.
...
1. Download the CRD ([waf-crd.yaml](../../crd/waf/waf-crd.yaml)).
