...
**Note:**
OpenShift support of alternate backends is now supported by Citrix ingress controller. Netscaler is configured according to the weights provided in the routes definition and traffic is distributed among the service pods based on those weights.
...
-  The user name and password of the Netscaler VPX or MPX appliance used as the Ingress device. If you are not using the default credentials, the Netscaler appliance must have a system user account with certain privileges so that the Citrix ingress controller can configure the Netscaler MPX, or VPX appliance. To create a system user account on Netscaler, see [Create a system user account for the Citrix ingress controller in Netscaler](https://github.com/netscaler/netscaler-k8s-ingress-controller/blob/master/docs/deploy/deploy-cic-yaml.md#create-system-user-account-for-citrix-ingress-controller-in-citrix-adc).

    After creating a system user for Netscaler, you can create an OpenShift secret using the following command:
