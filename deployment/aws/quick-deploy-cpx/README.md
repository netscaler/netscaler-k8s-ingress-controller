...
A [Linux machine with docker](https://docs.docker.com/v17.12/install/#supported-platforms).
...
Citrix ADC CPX can be configured using **cli_script.sh** or **NITRO API** calls:
...
An example to configure CPX to load-balance HTTP traffic between two apache containers on same Linux host.

   *  Pull apache container image on Linux host
   
        ```
        docker pull httpd:latest
        ```
   
   *  Deploy two apache containers using 'docker run' command
   
        ```
        root@ubuntu:~# docker run -dt --name Server-1 httpd:latest
        4688e72bd1eaa669f26310434481766e41f8dad76735c62c7fb2c8f946776230
        root@ubuntu:~# docker run -dt --name Server-2 httpd:latest
        f87bf3770bfe4c557e6cc5c5a26f03669d58a6f2419e8573f9aeb01b6751e540
        ```

   *  Finding out the IPs of apache containers using 'docker inspect' command
   
        ```
        root@ubuntu:~# docker inspect Server-1 | grep IPA
                "SecondaryIPAddresses": null,
                "IPAddress": "172.17.0.7",
                        "IPAMConfig": null,
                        "IPAddress": "172.17.0.7",
        root@ubuntu:~#
        root@ubuntu:~# docker inspect Server-2 | grep IPA
                "SecondaryIPAddresses": null,
   ...
   *  Configuring and verifying VIP and Services on CPX using **cli_script.sh**
